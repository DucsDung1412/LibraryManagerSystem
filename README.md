...
public JPanel panel_Home() {
		...
		panel_sapXep.setBounds(160, 92, 159, 90);
		...
		scrollPane_theLoai.setBounds(75, 92, 270, 150);
		...
		ArrayList<loaiSach> listSach = (ArrayList<loaiSach>) loaiSachDAO.getloaiSachDAO().selectAll();

		for (loaiSach loai : listSach) {
			...
			// loaiSachDAO.getloaiSachDAO().selectG().getTenLoaiSach();
		...
		panel_mainHome.setBorder(new LineBorder(new Color(0, 0, 0)));
		...
		Image imgsearch = imgTK.getScaledInstance(imgtimKiem_paneltopHome.getWidth(), imgtimKiem_paneltopHome.getHeight(),
				Image.SCALE_SMOOTH);
		...
		txttimKiem_paneltopHome.setColumns(10);
		...
		ArrayList<sach> list = (ArrayList<sach>) sachDAO.getsachDAO().selectAll();
		...
		for (sach ds : list) {
			...
			if (temp > 8 && temp % 3 == 0) {
				...
				panel_mainHome.setPreferredSize(new Dimension(scrollPane_mainHome.getWidth() - 22,
						scrollPane_mainHome.getHeight() + 700 * xx + 52));
			} else if (temp <= 8) {
				panel_mainHome.setPreferredSize(
						new Dimension(scrollPane_mainHome.getWidth() - 22, scrollPane_mainHome.getHeight() + 52));
			}
			...
			lbltenSach_panelsach.setFont(new Font("Arial", Font.PLAIN, 16));
			lbltenSach_panelsach.setBounds(20, 210, 289, 33);
			...
			btnxemThem_panelsach.setBounds(111, 255, 111, 21);
...
//					
			...
			imghinhSach_panelsach.setHorizontalAlignment(SwingConstants.CENTER);
			imghinhSach_panelsach.setBorder(new EmptyBorder(0, 0, 0, 0));
			imghinhSach_panelsach.setBounds(0, 0, 338, 119);
			...
			Image img = imgsach.getScaledInstance(imghinhSach_panelsach.getWidth(), imghinhSach_panelsach.getHeight(),
					Image.SCALE_SMOOTH);
			ImageIcon ne10 = new ImageIcon(img);
			imghinhSach_panelsach.setIcon(ne10);
